...
parser.add_argument("-m", "--mode",type=str, default='walk', help="specify mode of locomotion to run from config file") # jog mode before 
parser.add_argument("-f", "--file_path", type=str, default="simbicon_control/settings/config_flame.yml", help="path to config file (yml format)")
...
# env = Biped2dBullet();
env=gym.make("BipedalPyBulletEnv-v1")
env.render(mode="realtime")
env.robot.dt = args.sim_dt
env.mu = settings['mu']
...
#let robot fall on ground
for  i in range(int(0.4/env.robot.dt)):
    env.robot.p.stepSimulation()
print("robot has fall on ground")
time.sleep(1)
...
for i in range(int(max_iters)):
    ...
    #TODO: add to avoid large force
    omega_torso = np.clip(omega_torso,-3.14,3.14)
    state[2*(swing_ind+1)+1] = np.clip(state[2*(swing_ind+1)+1],-3.14,3.14)

    torque_stance = -kp_torso*(theta_torso-targ_ang[0]) - kd_torso*omega_torso #-0.065
    print("torque stance:",torque_stance,"= -%f*(%f-%f) - %f*%f"%(kp_torso,theta_torso,targ_ang[0],kd_torso,omega_torso))
    torque_stance += -env.kp[swing_ind]*(state[2*(swing_ind+1)]-targ_ang[swing_ind+1]) - env.kd[swing_ind]*state[2*(swing_ind+1)+1]
    print("torque stance:",torque_stance,"= --%f*(%f-%f) - %f*%f"\
        %(env.kp[swing_ind],state[2*(swing_ind+1)],targ_ang[swing_ind+1],env.kd[swing_ind],state[2*(swing_ind+1)+1]))
