import envs
from envs.bipedal_bullet_env_rl_flame8_a6 import BipedalBulletRLEnvFLAME8A6 
from envs.bipedal_bullet_env_rl_flame10_a6 import BipedalBulletRLEnvFLAME10A6 
from ray import tune
from ray.rllib.agents.ppo import PPOTrainer
from ray.tune.registry import register_env
import ray
import ray.rllib.agents.ppo as ppo
from ray.tune.logger import pretty_print
import gym
import time
import numpy as np
import pickle

env_name = 'BipedalPyBulletEnv-v11'
# instantiate env class
def env_creator(env_config):
    import envs
    return gym.make(env_name)  # return an env instance
register_env("my_env", env_creator)

#trainer config
ray.init()
config = ppo.DEFAULT_CONFIG.copy()
config["num_gpus"] = 0
config["num_workers"] = 1
config["explore"] = False
config["framework"] = "torch"
config["seed"] = 12345
config["num_sgd_iter"] = 20
config["kl_coeff"]=1.0
config["lr"] = 0.0001
config["sgd_minibatch_size"] =2560#32768
config["train_batch_size"] = 25600#320000
config["batch_mode"] = "complete_episodes"
config["observation_filter"]  = "MeanStdFilter"
trainer = ppo.PPOTrainer(config=config, env="my_env")
restore_path = "/root/ray_results/PPO_my_env_2021-08-23_22-00-43bnev_f58/checkpoint_009886/checkpoint-9886"
trainer.restore(restore_path) 

# run until episode ends
env = gym.make(env_name)
for i in range(100):
    episode_reward = 0
    done = False
    env.render()
    obs = env.reset()

    save_dict = {
        "state_list": [],
        "action_list": [] 
    }
    
    while not done:
        action = trainer.compute_action(obs)
        print(obs)

        #save data
        real_action = np.array([0.0,action[0],action[1],action[2],action[3],action[4],action[5]])
        save_dict["state_list"].append(obs)
        save_dict["action_list"].append(real_action)

        obs, reward, done, info = env.step(action)
        episode_reward += reward
        # time.sleep(0.1)
    with open('example/data/exp4_trajectory.pkl', 'wb') as f:
        save_dict["state_list"] = np.asarray(save_dict["state_list"])
        save_dict["action_list"] = np.asarray(save_dict["action_list"])
        pickle.dump(save_dict, f)
    break
    print("episode reward:",episode_reward)
