...
import numpy as np
import gym
class BipedalBulletRLEnvFLAME8A6(BipedalBaseEnv):
    def __init__(self):
        self.robot = BipedalRobot2D(file_path="urdf/simbicon_urdf/flame8.urdf",delta_t=0.01)
        ...
        obs_dim = 24
        high = np.ones([action_dim]) * 30.0 #5
    ...
    def calc_reward(self,state):
        ...
        foot_weight = 0.02 #0.02 remove this reward
        foot_threshold  = 0.2
        parallelfoot_reward = (foot_threshold - np.mean(np.absolute(np.asarray(foots_rpy)))  )
        parallelfoot_reward = 0.0 if parallelfoot_reward < 0 else foot_weight
        ...
        if(walk_progress_x<0.0):
            alive_rate = 0.1
            speed_reward = 0.0
        walk_progress_cost = walk_progress_x * 10.0
        alive_bonus =  alive*self.robot.dt*alive_rate
        

        self.rewards=[walk_progress_cost,alive_bonus,double_stand_reward,statemachine_reward,speed_reward,parallelfoot_reward,knee_reward]
