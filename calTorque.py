...
class multijoint_robot():
    def __init__():
        self.upperbody = joint
...
#read txt file function
    ...
    def assignRef(self):
        center_hip.q.ref = networkOutput(robot,weight,offset,scale,1)
        upperbody.q.ref = networkOutput(robot,weight,offset,scale,2)
        interleg.q.ref = networkOutput(robot,weight,offset,scale,3)
        stance_knee.q.ref = networkOutput(robot,weight,offset,scale,4)
        swing_knee.q.ref = networkOutput(robot,weight,offset,scale,5)
        stance_ankle.q.ref = networkOutput(robot,weight,offset,scale,6)
        swing_ankle.q.ref = networkOutput(robot,weight,offset,scale,7)
        kp=weight[26,-7:]
        kd=weight[27,-7:]
    ...
    def FiltandDerivRef(joint, dt):
        ref = joint.q.ref
        refPrev = joint.q.refPrev
        ref = alpha * ref +  (1.0-alpha)*refPrev
        joint.qd.ref = (ref - refPrev)/dt
        refPrev = copy.deepcopy(ref)


    # calTau
    def update(robot):
        #define centerhipx, interleg, stance_knee, swing knee, stance ankle, swing ankle///what is upperbody???
            left_is_stance = True
        if left_is_stance:
            stance_leg = 'left'
            swing_leg = 'right'
        ...
        robot.swing_hipy.tau += robot.interleg.tau
        robot.stance_hipy.tau -= robot.interleg.tau
        robot.stance_hipy.tau -= robot.upperbody.tau
